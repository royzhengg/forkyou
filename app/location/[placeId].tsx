...
import {
  ...
  Image, Linking, useWindowDimensions, ActivityIndicator, ActionSheetIOS, Modal,
} from 'react-native'
...
import {
  ChevronLeft, NavIcon, BookmarkIcon, PhoneIcon, GlobeIcon,
  PinIcon, ClockIcon, ChevronDown, SortIcon, ImagePlaceholder,
...
const SORT_LABELS: Record<PostSort, string> = { liked: 'Most liked', newest: 'Newest', oldest: 'Oldest' }
...
function todayHoursIndex(): number {
...
export default function LocationInfoScreen() {
  ...
  // Compute forkyou averages from contextPosts (fall back to Supabase if empty)
  const forkyouRatings = useMemo(() => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (placeResult) {
        ...
        const refs = (placeResult.photos ?? []).slice(0, 6).map((p: { photo_reference: string }) => p.photo_reference)
        setPhotoUrls(refs.map((ref: string) =>
      ...
      if (resId) {
        ...
        if (ratingsRes.data && ratingsRes.data.length > 0) {
          const rows = ratingsRes.data as { food_rating: number; vibe_rating: number; cost_rating: number }[]
          const avg = (key: keyof typeof rows[0]) => {
            const vals = rows.map(r => r[key]).filter((v): v is number => v != null)
            return vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : null
          }
          setDbRatings({ food: avg('food_rating'), vibe: avg('vibe_rating'), cost: avg('cost_rating') })
        }
  ...
  async function findOrCreateRestaurant(): Promise<string | null> {
    ...
    const { data: existing } = await (supabase.from('restaurants') as any)
      .select('id').eq('google_place_id', pid).maybeSingle()
    if (existing?.id) { setRestaurantId(existing.id); return existing.id }
  ...
  const toggleSave = useCallback(async () => {
    ...
    if (!resId) { setSaved(wasSaved); return }
    if (wasSaved) {
      const { error } = await (supabase.from('saved_locations') as any)
        .delete().eq('user_id', user.id).eq('restaurant_id', resId)
      if (error) setSaved(wasSaved)
    } else {
      const { error } = await (supabase.from('saved_locations') as any)
        .insert({ user_id: user.id, restaurant_id: resId })
      if (error) { setSaved(wasSaved); return }
      setSaveSheet(true)
    }
  ...
  const goToMap = useCallback(() => {
    router.push({
      ...
      params: {
        ...
        avgFood: forkyouRatings.food != null ? String(forkyouRatings.food.toFixed(1)) : '',
        avgVibe: forkyouRatings.vibe != null ? String(forkyouRatings.vibe.toFixed(1)) : '',
        avgCost: forkyouRatings.cost != null ? String(forkyouRatings.cost.toFixed(1)) : '',
        photoUrl: photoUrls[0] ?? '',
        todayHours: detail?.opening_hours?.weekday_text?.[todayHoursIndex()] ?? '',
      },
    })
  }, [router, placeId, name, lat, lng, detail, forkyouRatings, photoUrls])
  ...
  const hasForkyouRatings = forkyouRatings.food != null || forkyouRatings.vibe != null || forkyouRatings.cost != null
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.infoSection}>
            ...
            {/* Ratings card */}
            {(hasGoogleRating || hasForkyouRatings) && (
              <View style={styles.ratingsCard}>
                  ...
                  <View style={styles.ratingsCardRow}>
                    ...
                    <View style={styles.ratingsCardValues}>
                      ...
                      {detail!.user_ratings_total != null && (
                        <Text style={styles.ratingCount}>{detail!.user_ratings_total.toLocaleString()} reviews</Text>
                      )}
                    </View>
                  </View>
                )}
                ...
                {hasForkyouRatings && (
                  <View style={styles.ratingsCardRow}>
                    <Text style={styles.ratingsCardLabel}>forkyou</Text>
                    <View style={styles.ratingsCardValues}>
                      {forkyouRatings.food != null && (
                        <View style={styles.ratingChip}>
                          <Text style={styles.ratingChipLabel}>FOOD</Text>
                          <Stars count={Math.round(forkyouRatings.food)} size={12} />
                        </View>
                      )}
                      {forkyouRatings.vibe != null && (
                        <View style={styles.ratingChip}>
                          <Text style={styles.ratingChipLabel}>VIBE</Text>
                          <Stars count={Math.round(forkyouRatings.vibe)} size={12} />
                        </View>
                      )}
                      {forkyouRatings.cost != null && (
                        <View style={styles.ratingChip}>
                          <Text style={styles.ratingChipLabel}>COST</Text>
                          <Dollars count={Math.round(forkyouRatings.cost)} size={11} />
                        </View>
                      )}
                      {contextPosts.length > 0 && (
                        <Text style={styles.ratingCount}>{contextPosts.length} post{contextPosts.length !== 1 ? 's' : ''}</Text>
                      )}
                    </View>
                  </View>
                )}
            ...
            <View style={styles.contactSection}>
              ...
              {!!detail?.formatted_phone_number && (
                <TouchableOpacity style={styles.contactRow} onPress={() => openPhone(detail!.formatted_phone_number!)}>
                  <PhoneIcon />
                  <Text style={styles.contactText}>{detail.formatted_phone_number}</Text>
                </TouchableOpacity>
              )}
              {!!detail?.website && (
                <TouchableOpacity style={styles.contactRow} onPress={() => openWebsite(detail!.website!)}>
                  <GlobeIcon />
                  <Text style={styles.contactTextLink} numberOfLines={1}>{detail.website.replace(/^https?:\/\//, '').replace(/\/$/, '')}</Text>
                </TouchableOpacity>
              )}
                  ...
                  <View style={{ flex: 1 }}>
                    {hoursExpanded ? (
                      weekdayText.map((line, i) => (
                        <Text key={i} style={[styles.contactText, i === todayIdx && styles.contactTextBold]}>
                          {line}
                        </Text>
                      ))
                    ) : (
          ...
          <View style={styles.postsSection}>
            <View style={styles.postsSectionHeader}>
              <Text style={styles.postsSectionTitle}>
                {contextPosts.length > 0
                  ? `${contextPosts.length} post${contextPosts.length !== 1 ? 's' : ''} on forkyou`
                  : 'Posts on forkyou'}
              </Text>
      ...
      <Modal visible={saveSheet} transparent animationType="fade" onRequestClose={() => setSaveSheet(false)}>
        ...
        <View style={styles.sheet}>
          ...
          <TouchableOpacity
            ...
            onPress={() => { setSaveSheet(false); router.push('/(tabs)/places') }}
            activeOpacity={0.8}
          >
...
async function fetchPlaceDetail(placeId: string): Promise<PlacesDetail | null> {
  ...
  try {
    const fields = 'rating,user_ratings_total,formatted_phone_number,website,opening_hours,price_level,photos,types,business_status,geometry'
...
function makeStyles(c: ReturnType<typeof useThemeColors>) {
  return StyleSheet.create({
    ...
    header: { height: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, borderBottomWidth: 0.5, borderBottomColor: c.border },
    ...
    photoPlaceholder: { height: PHOTO_HEIGHT, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center', gap: 8 },
    ...
    ratingsCard: { backgroundColor: c.surface, borderRadius: 12, marginBottom: 20, overflow: 'hidden' },
    ratingsCardRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12, gap: 12 },
    ratingsCardLabel: { fontSize: 11, fontWeight: '600', color: c.text3, width: 52 },
    ratingsCardValues: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' },
    ...
    postsSectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, marginBottom: 12 },
    ...
    postRow: { flexDirection: 'row', gap: 12, paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: 0.5, borderBottomColor: c.border },
    postThumb: { width: 60, height: 60, borderRadius: 8, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    ...
    sheet: { backgroundColor: c.bg, borderTopLeftRadius: 20, borderTopRightRadius: 20, borderTopWidth: 0.5, borderTopColor: c.border, paddingHorizontal: 24, paddingTop: 14, paddingBottom: 40, alignItems: 'center', gap: 10 },
    sheetHandle: { width: 36, height: 4, borderRadius: 2, backgroundColor: c.border2, marginBottom: 6 },
    sheetIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center', marginBottom: 2 },
    sheetTitle: { fontSize: 17, fontWeight: '600', color: c.text },
    sheetBody: { fontSize: 13, color: c.text2, textAlign: 'center', lineHeight: 19 },
    sheetBtnPrimary: { width: '100%', backgroundColor: c.text, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginTop: 6 },
    sheetBtnPrimaryText: { fontSize: 14, fontWeight: '600', color: c.bg },
    sheetBtnSecondary: { width: '100%', backgroundColor: c.surface, borderWidth: 0.5, borderColor: c.border2, borderRadius: 14, paddingVertical: 14, alignItems: 'center' },
