...
function EyeIcon({ open }: { open: boolean }) {
  const colors = useThemeColors()
  return (
    <Svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={colors.text3} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
      {open ? (
        <>
...
export default function ChangePasswordScreen() {
  ...
  async function handleSave() {
    ...
    if (!user?.email) { setError('Unable to verify current account.'); setLoading(false); return }
      ...
      <View style={styles.form}>
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={styles.inputWrap}>
            <TextInput style={[styles.input, { flex: 1 }]} value={currentPassword} onChangeText={setCurrentPassword} placeholder="Current password" placeholderTextColor={colors.text3} secureTextEntry={!showCurrent} />
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={styles.inputWrap}>
            <TextInput style={[styles.input, { flex: 1 }]} value={newPassword} onChangeText={setNewPassword} placeholder="At least 8 characters" placeholderTextColor={colors.text3} secureTextEntry={!showNew} />
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.inputWrap, !passwordsMatch && confirmPassword.length > 0 && styles.inputError]}>
            <TextInput style={[styles.input, { flex: 1 }]} value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Confirm new password" placeholderTextColor={colors.text3} secureTextEntry={!showConfirm} />
            <TouchableOpacity onPress={() => setShowConfirm(v => !v)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
              <EyeIcon open={showConfirm} />
            </TouchableOpacity>
          </View>
          {!passwordsMatch && confirmPassword.length > 0 && <Text style={styles.matchError}>Passwords don't match</Text>}
        </View>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
        >
          {loading ? <ActivityIndicator size="small" color={colors.bg} /> : <Text style={styles.primaryBtnText}>Update password</Text>}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  )
...
function makeStyles(c: ReturnType<typeof useThemeColors>) {
  return StyleSheet.create({
    ...
    inputWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: c.surface, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1, borderColor: 'transparent' },
